...
**Hard constraints**: the image installs only `onnxruntime` + `tokenizers` at BUILD time for local NER/sentiment/router models; the old "stdlib-only agent / no pip installs" rule was self-imposed, not a competition constraint. Image must stay well under 5 GB compressed. Everything the container needs is baked at build time (`HF_HUB_OFFLINE=1`); the only permitted runtime egress is the Fireworks proxy — any other network routing disqualifies the submission.
